...
export interface CardPaymentAck {
     ...
     * (±₹1 only). CRED rewards mismatch stays unpaid for Home mark-paid.
     * Never use the savings last4 (XX412 is 1412, not ICICI 0004).
     */
...
/** Home default `Since` and the daily Telegram digest lookback. */
export const ATTENTION_DUE_LOOKBACK_MONTHS = 6;

/**
 * Inclusive IST start of the Attention due window (`today` minus `months`).
 *
 * @param today - `YYYY-MM-DD` IST
 * @param months - Calendar months to look back (Home default is 6)
 */
export function attentionDueLookbackStart(
    today: string = todayIstDate(),
    months: number = ATTENTION_DUE_LOOKBACK_MONTHS
): Date {
    const match = today.trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
    ...
    const last = new Date(Date.UTC(startYear, startMonth, 0)).getUTCDate();
    const startDay = Math.min(day, last);
    const iso = `${startYear}-${String(startMonth).padStart(2, "0")}-${String(startDay).padStart(2, "0")}`;

    return new Date(`${iso}T00:00:00+05:30`);
}

/**
 * True when the bill's due day (or SMS day if undated) sits in `[from, to]`.
 * Same window Home uses for `?from=` / `?to=`.
 *
 * @param row - Due date and SMS time
 * @param from - Inclusive start
 * @param to - Inclusive end
 */
export function isDueInLookback(
    row: { dueDate?: string | null; occurredAt: Date },
    from?: Date,
    to?: Date
): boolean {
    if (!from && !to) {
    ...
    const dueDay = row.dueDate?.trim().slice(0, 10);
    const day =
        dueDay && /^\d{4}-\d{2}-\d{2}$/.test(dueDay) ? dueDay : todayIstDate(row.occurredAt);
 ...
 * UPI from savings settle a card due only when the amount matches (±₹1).
 * CRED rewards/discount mismatches stay unpaid so Home mark-paid can be used.
 * The payee must not name a different bank. Prefer the due whose
