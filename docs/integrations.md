...
- **Daily** at 08:00 IST: unpaid dues; Dhan this month vs last month income/expense; SMS **spend buckets** (groceries, dining, shopping, …) and top/large merchants vs the same days last month. No balances. Not on Home.
...
Narada maps owned last4 accounts, skips rows before each account's ledger opening, and POSTs withdrawals / deposits / transfers. Withdrawals include `category_name` from the SMS merchant (Groceries, Dining, Shopping, …); Firefly creates the category if it is missing. Push is idempotent via `external_id` (`narada-sms-{smsId}`). Already-pushed journal ids are kept on `financial_events` when events are rebuilt. Old uncategorized Dhan rows are not backfilled.
