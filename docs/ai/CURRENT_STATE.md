...
✅ Due feed: `GET /knowledge?kind=due` unique last4+dueDate+amount (undated still unique on last4+amount in the same SMS month); Airtel WiFi/Fixedline collapse as one broadband biller; IGL `Payment of Rs … is pending against` BP reminders are dues (same-month IGL amount collapses); paid when a received/credited SMS hits that last4 in-cycle, or when an IGL confirmation / IGL merchant spend matches that IGL cycle (same amount only); IGL `pending against` still lists if analysis cashFlow is wrong; ₹0 outstanding omitted; overdue only if due date passed and unpaid
