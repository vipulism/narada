...
✅ Due feed: `GET /knowledge?kind=due` unique last4+dueDate+amount (undated still unique on last4+amount; Airtel WiFi/Fixedline are one biller); paid when a received/credited SMS hits that last4 in-cycle; ₹0 outstanding omitted; overdue only if due date passed and unpaid
