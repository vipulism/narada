...
✅ Merchant spend categories at `GET /merchants.html` (`merchant_categories`; builtin Education plus user-created `spend_buckets`; SMS preview can override one SMS category or move it to another merchant item via `sms_spend_overrides`; rename / merge via `merchant_aliases`; list search / Show / Sort with newest SMS first; Zerodha/ICCL/Indian Clearing/Groww ACH funding is investment, not a spend row; HDFC SI Hub AutoPay Success is Netflix/LinkedIn subscriptions; truncated IKEA UPI VPA is shopping; HDFC `At _TITAN COMPANY LI..` POS is catalogued as Tanishq (shopping); ICICI `Indraprastha Ga` UPI and IGL PNG receipts are catalogued as IGL (utilities); Apply-this-SMS says before ledger opening when the SMS is pre-16 Aug 2026; Dhan `category_name` on new withdrawals and on apply for already-pushed journals)
